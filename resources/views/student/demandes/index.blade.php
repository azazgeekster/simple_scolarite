...
@section("main_content")
    <div class="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
            ...
            <!-- Request Form Card -->
            <div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200/50 dark:border-gray-700/50 p-6 sm:p-8 mb-8 transition-all duration-300 hover:shadow-2xl">
                ...
                <form method="POST" action="{{ route('student.demande.store') }}" class="space-y-6">
                    @csrf
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        <!-- Document Select -->
                        <div class="group">
                            <label for="document_id" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">
                            ...
                            <select name="document_id" id="document_id" required
                                ...
                                @foreach($documents as $doc)
                                    <option value="{{ $doc->id }}">{{ $doc->label_fr }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <!-- Retrait Type Select -->
                        <div class="group">
                            <label for="retrait_type" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">
                                <span class="flex items-center space-x-2">
                            ...
                            <select name="retrait_type" id="retrait_type" required
                                ...
                                <option disabled {{ old('retrait_type') === null ? 'selected' : '' }}>Choisissez le type</option>
                                <option value="temporaire" {{ old('retrait_type') === 'temporaire' ? 'selected' : '' }}>Temporaire</option>
                                <option value="permanent" {{ old('retrait_type') === 'permanent' ? 'selected' : '' }}>Permanent</option>
                            </select>
                        </div>
            ...
            <div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200/50 dark:border-gray-700/50 overflow-hidden">
                ...
                <div class="block lg:hidden">
                    @forelse($demandes as $demande)
                        <div class="p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700 last:border-b-0 hover:bg-gray-50/50 dark:hover:bg-gray-700/50 transition-colors duration-200">
                            <div class="space-y-4">
                                <!-- Header -->
                                ...
                                <div class="grid grid-cols-2 gap-4 text-sm">
                                    ...
                                    <div class="space-y-1">
                                        ...
                                        <p class="text-gray-900 dark:text-white">{{ $demande->created_at->format('d/m/Y H:i') }}</p>
                                    </div>
                                ...
                                <div class="flex flex-wrap gap-2 pt-2">
                                    @if($demande->status === 'PENDING')
                                        <form action="{{ route('student.demande.cancel', $demande->id) }}" method="POST"
                                            onsubmit="return confirm('Êtes-vous sûr de vouloir annuler cette demande ?')" class="inline">
                                            @csrf
                                            @method('DELETE')
                                    ...
                                    <a href="{{ route('student.demande.print', $demande->id) }}" title="Imprimer" target="_blank"
                    ...
                    @empty
                        <div class="p-8 text-center">
                            ...
                            <p class="text-gray-400 dark:text-gray-500 text-sm mt-1">Vos demandes apparaîtront ici une fois soumises.</p>
                        </div>
                    @endforelse
                </div>
                    ...
                    <table class="min-w-full">
                        <thead class="bg-gray-50 dark:bg-gray-700/50">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">An. académique</th>
                                <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Fait le</th>
                                <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Retour avant</th>
                                <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Extension</th>
                                <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Retourné le</th>
                            ...
                            @forelse($demandes as $demande)
                                <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-700/25 transition-colors duration-200">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="font-medium text-gray-900 dark:text-white">{{ $demande->document->label_fr }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="capitalize text-gray-700 dark:text-gray-300">{{ $demande->retrait_type }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-gray-900 dark:text-white">{{ $demande->academicYear->label }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ $demande->created_at->format('d/m/Y H:i') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if($demande->retrait_type === 'temporaire' && $demande->must_return_by)
                                ...
                                <tr>
                                    <td colspan="9" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-gray-500 dark:text-gray-400 text-lg font-medium">Aucune demande enregistrée.</p>
                                            <p class="text-gray-400 dark:text-gray-500 text-sm mt-1">Vos demandes apparaîtront ici une fois soumises.</p>
                                        </div>
                                    </td>
                                </tr>
