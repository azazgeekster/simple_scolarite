        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-4">
                @php
                    ...
                    $unreadNotifications = $student ? $student->unreadNotifications()->take(5)->get() : collect();
                    $unreadCount = $student ? $student->unreadNotifications()->count() : 0;
                @endphp
                ...
                <div class="z-20 z-50 hidden max-w-sm my-4 overflow-hidden text-base list-none bg-white divide-y divide-gray-100 rounded shadow-lg dark:divide-gray-600 dark:bg-gray-700"
                    ...
                    <div
                        ...
                        Notifications
                        @if($unreadCount > 0)
                            <span class="ml-2 px-2 py-1 text-xs font-medium bg-red-100 dark:bg-red-900/50 text-red-800 dark:text-red-200 rounded-full">{{ $unreadCount }} {{ $unreadCount > 1 ? 'nouvelles' : 'nouvelle' }}</span>
                        @endif
                    </div>
                    <div class="max-h-96 overflow-y-auto">
                        @forelse($unreadNotifications as $notification)
                            @php
                                ...
                                $color = $priorityColors[$data['priority'] ?? 'normal'] ?? 'bg-blue-500';
                            @endphp
                            <a href="{{ route('student.messages.show', $data['message_id']) }}"
                                class="flex px-4 py-3 border-b hover:bg-gray-100 dark:hover:bg-gray-600 dark:border-gray-600">
                                <div class="flex-shrink-0">
                                    <div class="w-11 h-11 {{ $color }} rounded-full flex items-center justify-center">
                                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div class="w-full pl-3">
                                    <div class="text-gray-500 font-normal text-sm mb-1.5 dark:text-gray-400">
                                        <span class="font-semibold text-gray-900 dark:text-white">{{ $data['subject'] ?? 'Nouveau message' }}</span>
                                        @if(isset($data['sender_name']))
                                            <br><span class="text-xs">De: {{ $data['sender_name'] }}</span>
                                        @endif
                                    </div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="px-2 py-0.5 text-xs font-medium rounded-full {{ $data['priority'] === 'urgent' ? 'bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-200' : ($data['priority'] === 'high' ? 'bg-orange-100 text-orange-800 dark:bg-orange-900/50 dark:text-orange-200' : 'bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-200') }}">
                                            {{ ucfirst($data['category'] ?? 'général') }}
                                        </span>
                                    </div>
                                </div>
                            </a>
