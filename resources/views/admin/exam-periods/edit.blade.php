...
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Modifier la Période d'Examens</h1>
    ...
    <!-- Warning if exams are attached -->
    @if($examPeriod->exams()->exists())
        <div class="p-4 mb-4 text-sm text-yellow-800 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-400" role="alert">
            <span class="font-medium">Attention!</span> Cette période contient {{ $examPeriod->exams()->count() }} examen(s).
            Modifier l'année/saison/session affectera ces examens.
        </div>
    @endif
            ...
            <form action="{{ route('admin.exam-periods.update', $examPeriod) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <select name="academic_year"
                            ...
                            @foreach($academicYears as $year)
                                <option value="{{ $year->start_year }}"
                                    {{ (old('academic_year', $examPeriod->academic_year) == $year->start_year) ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="season"
                                ...
                                required
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                            <option value="autumn" {{ old('season', $examPeriod->season) == 'autumn' ? 'selected' : '' }}>
                                Automne
                            </option>
                            <option value="spring" {{ old('season', $examPeriod->season) == 'spring' ? 'selected' : '' }}>
                                Printemps
                            </option>
                        </select>
                        @error('season')
                    ...
                    <div>
                        ...
                        <select name="session_type"
                                ...
                                required
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                            <option value="normal" {{ old('session_type', $examPeriod->session_type) == 'normal' ? 'selected' : '' }}>
                                Session Normale
                            </option>
                            <option value="rattrapage" {{ old('session_type', $examPeriod->session_type) == 'rattrapage' ? 'selected' : '' }}>
                                Session Rattrapage
                            </option>
                        </select>
                        @error('session_type')
                    ...
                    <div>
                        <label for="label" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                            Libellé <span class="text-gray-400 text-xs">(optionnel)</span>
                        </label>
                        <input type="text"
                               name="label"
                               id="label"
                               value="{{ old('label', $examPeriod->label) }}"
                    ...
                    <div>
                        ...
                        <input type="date"
                               name="start_date"
                               id="start_date"
                               value="{{ old('start_date', $examPeriod->start_date->format('Y-m-d')) }}"
                               required
                               class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                        @error('start_date')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="date"
                               name="end_date"
                               id="end_date"
                               value="{{ old('end_date', $examPeriod->end_date->format('Y-m-d')) }}"
                               required
                               class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                        @error('end_date')
                            <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="description"
                              ...
                              class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white">{{ old('description', $examPeriod->description) }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mt-6 space-y-4">
                    ...
                    <div class="flex items-start">
                        <div class="flex items-center h-5">
                            <input type="checkbox"
                                   ...
                                   value="1"
                                   {{ old('auto_publish_exams', $examPeriod->auto_publish_exams) ? 'checked' : '' }}
                    ...
                    <div class="flex items-start">
                        ...
                        <div class="ml-3 text-sm">
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400">
                                Activer cette période (désactivera automatiquement les autres périodes du même type)
                            </p>
                        </div>
                    </div>
                </div>

                <!-- Stats Box (if exams exist) -->
                @if($examPeriod->exams()->exists())
                    @php
                        $stats = $examPeriod->getExamStats();
                    @endphp
                    <div class="mt-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                        <h3 class="text-sm font-medium text-gray-900 dark:text-white mb-3">Statistiques de cette période:</h3>
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                            ...
                            <div>
                                ...
                                <div class="text-lg font-bold text-green-700 dark:text-green-300">{{ $stats['published'] }}</div>
                            </div>
                            <div>
                                <div class="text-xs text-yellow-600 dark:text-yellow-400">Non Publiés</div>
                                <div class="text-lg font-bold text-yellow-700 dark:text-yellow-300">{{ $stats['unpublished'] }}</div>
                            </div>
                            <div>
                                <div class="text-xs text-blue-600 dark:text-blue-400">Étudiants</div>
                                <div class="text-lg font-bold text-blue-700 dark:text-blue-300">{{ $stats['students'] }}</div>
                            </div>
                        </div>
                    </div>
                @endif
